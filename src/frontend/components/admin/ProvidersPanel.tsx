...
import type { ProviderConfig, SystemConfig } from "@/lib/api/types";
import { saveProviderConfig, saveSystemConfigs } from "@/app/admin/actions";
...
function buildKeyLists(data: ProviderConfig[]) {
  const listsMap: { [key: string]: string[] } = {};
  data.forEach((c) => {
    const count = (c as ProviderConfig & { key_count?: number }).key_count || (c.has_key ? 1 : 0);
    listsMap[c.id] = c.has_key
      ? count > 0
        ? Array(count).fill("__NO_CHANGE__")
        : ["__NO_CHANGE__"]
      : [""];
  });
  return listsMap;
...
function buildSystemKeys(sysData: SystemConfig[]) {
  const sysKeysMap: { [key: string]: string } = {};
  sysData.forEach((c) => {
    sysKeysMap[c.key] = c.is_encrypted
      ? c.has_value
        ? "__NO_CHANGE__"
        : ""
      : c.value || "";
...
interface ProvidersPanelProps {
  ...
  initialSystemConfigs: SystemConfig[];
}

export function ProvidersPanel({
  initialConfigs,
  initialSystemConfigs,
}: ProvidersPanelProps) {
  const [configs, setConfigs] = useState<ProviderConfig[]>(initialConfigs);
  const [systemConfigs, setSystemConfigs] = useState<SystemConfig[]>(initialSystemConfigs);
  ...
  const { consoleLogs, consoleContainerRef, addLog } = useAdminConsole(
    `[${new Date().toLocaleTimeString()}] LOAD: ${initialConfigs.length} providers (server cache).`
  );
  const { refresh } = useAdminRefresh();

  const [providerKeyLists, setProviderKeyLists] = useState<{ [key: string]: string[] }>(
    () => buildKeyLists(initialConfigs)
  );
  const [systemKeys, setSystemKeys] = useState<{ [key: string]: string }>(() =>
    buildSystemKeys(initialSystemConfigs)
  );

  useSyncProp(initialConfigs, setConfigs);
  useSyncProp(initialSystemConfigs, setSystemConfigs);

  const syncFromServer = useCallback((data: ProviderConfig[], sysData: SystemConfig[]) => {
    setProviderKeyLists(buildKeyLists(data));
    setSystemKeys(buildSystemKeys(sysData));
  }, []);

  useEffect(() => {
    syncFromServer(initialConfigs, initialSystemConfigs);
  }, [initialConfigs, initialSystemConfigs, syncFromServer]);
  ...
  const handleDeleteConfig = async (id: string, providerName: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/providers/${id}`, {
        method: "DELETE",
      });
